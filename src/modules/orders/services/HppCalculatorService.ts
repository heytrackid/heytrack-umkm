...
export class HppCalculatorService {
  ...
  private async calculateLaborCost(recipeId: string): Promise<number> {
    try {
      ...
      const totalLaborCost = productions.reduce((sum: number, p: any) => sum + Number(p.labor_cost), 0);
      const totalQuantity = productions.reduce((sum: number, p: any) => sum + Number(p.actual_quantity), 0);
  ...
  private async calculateOverheadCost(recipeId: string): Promise<number> {
    try {
      ...
      const totalOverhead = operationalCosts.reduce((sum: number, cost: any) => sum + Number(cost.amount), 0);
  ...
  private async calculateWacAdjustment(recipeId: string, _currentMaterialCost: number): Promise<number> {
    try {
      ...
      for (const ingredientId of ingredientIds) {
        ...
        const totalQuantity = ingredientTransactions.reduce((sum: number, t: any) => sum + Number(t.quantity), 0);
        const totalValue = ingredientTransactions.reduce((sum: number, t: any) => sum + Number(t.total_value), 0);
