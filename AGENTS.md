...
## Sen
try Error Monitoring & Performance Tracking

### Configuration Files
- **Client-side**: `src/instrumentation-client.ts` - Browser initialization
- **Server-side**: `sentry.server.config.ts` - Node.js initialization  
- **Edge**: `sentry.edge.config.ts` - Edge runtime initialization
- **Note**: Initialization only happens in these files. Use `import * as Sentry from "@sentry/nextjs"` in other files to reference Sentry functionality.
...
// Fatal level - severe errors causing application shutdown
...
- Use console logging integration for automatic capture

#### ❌ DON'T
- Don't initialize Sentry multiple times (only in config files)
- Don't create spans for trivial operations
- Don't log sensitive user data (PII)
- Don't use console.log directly (use Sentry logger instead)
- Don't forget to add error context with attributes
...
export function MyComponent() {
  ...
  const handleAction = async () => {
    try {
      ...
      // Also handle with existing error handler
